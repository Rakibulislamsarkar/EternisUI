...
function useFloatingActionPanelLogic() {
  ...
  const openPanel = React.useCallback((rect: DOMRect, newMode: "actions" | "note") => {
    setTriggerRect(rect)
    setMode(newMode)
    setIsOpen(true)
  }, [])
...
>(({ children, className }, ref) => {
  ...
  const getPosition = React.useCallback(() => {
    ...
    const top = triggerRect.bottom + scrollY + 8
...
export {
  ...
  type FloatingActionPanelContextType,
}
