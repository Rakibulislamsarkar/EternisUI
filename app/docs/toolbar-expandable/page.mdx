export const metadata = {
  title: 'Toolbar Expandable - Motion-Primitives',
  description:
    'Changes height based on content, expanding to display more tools and shrinking to save space. Keeps your screen organized and efficient.',
};

import ToolbarExpandable from '@/components/core/toolbar-expandable';
import ComponentCodePreview from '@/components/website/component-code-preview';

# Toolbar Dynamic

A toolbar that changes its height as needed. It grows to show more content and shrinks to save space, keeping your screen neat.

<ComponentCodePreview
  component={<ToolbarExpandable />}
  filePath='components/core/toolbar-expandable.tsx'
/>

**Please add:**

- [useClickOutside](https://github.com/Rakibulislamsarkar/EternisUI/blob/main/hooks/useClickOutside.tsx)
- [cn](https://github.com/Rakibulislamsarkar/EternisUI/blob/main/lib/utils.ts)
